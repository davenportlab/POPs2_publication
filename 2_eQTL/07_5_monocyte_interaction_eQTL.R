# 07_5_monocyte_interaction_eQTL.R

################################################################################

# 7.5. Run eQTL interaction with monocytes 

################################################################################

# Aim: Identify eQTL with effects varying by monocyte proportion 
  ...
  if (length(unique(individual[which(geno.int[, pairs.int[i, 2]] == 2
    ...
    if(nrow(signals) == 1){
      ...
      model.test <- capture_warnings_and_output(lmer(as.numeric(gene_exp[gene, ]) ~ # gene expression
                                                       ...
                                                       covariates + # covariates
                                                       peer_factors[, 1:n.peer] + # peer factors
                                                       geno.int[, as.character(pairs.int[i, 2])]*covariates[,2] + # interaction term 
                                                       (1|individual) + (1|batch), # individual and batch 
                                                     subset = complete.cases(geno.int[, as.character(pairs.int[i, 2])]),
                                                     REML=FALSE))
    } else{ 
    ...
    c(summary(model.test$result)$coefficients[1,], # get results info for the genotype
      summary(model.test$result)$coefficients["covariatesMonocytes_centered",], # get results for neutrophils
      summary(model.test$result)$coefficients["geno.int[, as.character(pairs.int[i, 2])]:covariates[, 2]",], # get interaction results for neutrophils
...
saveRDS(results, paste0(dir, "output_data/interactions_flanders_2/monocyte_int_results_flanders_2.rds"))
