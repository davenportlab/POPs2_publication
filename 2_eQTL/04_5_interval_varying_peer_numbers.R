# 04_5_interval_varying_peer_numbers.R

################################################################################

# 4.5. Compare eQTL results after including increasing number of PEER factors in the model

################################################################################

# Aim: Decide n peer factors to include in main effects eQTL model for interval FRA

########################### Output paths ##########################

########################### Input paths ###########################
inpath <- "interval_rna/POPS2_comparison/peertests/"
...
for(i in n.peer){
  cat.rds(paste0(inpath, "peertest", i, "/chr1/"), "*.rds",
          paste0(inpath, "chr1_results_", i, "peer_factors.rds"))
}
...
for(i in n.peer){
  print(i)
  eqtl <- readRDS(paste0(inpath, "chr1_results_", i, "peer_factors.rds"))
...
write.table(peer.res, paste0(inpath, "chr1_eqtl_number_by_peer.txt"), sep="\t")

########################### Plot ###########################
peer.res <- read.delim(paste0(inpath, "chr1_eqtl_number_by_peer.txt"))
...
# visualize varying of PEER factors (Supplementary Figure 33D)
