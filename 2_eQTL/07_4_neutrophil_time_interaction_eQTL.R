# 07_4_neutrophil_time_interaction_eQTL.R

################################################################################

# 7.4. Run eQTL interaction with time controlling for neutrophils to see if they explain time interactions

################################################################################

# Aim: Identify eQTL with effects varying by time not explained by neutrophil proportion 
  ...
  if ((length(unique(individual[which(geno.int[, pairs.int[i, 2]] == 2 # upper half of cell prop dist
    ...
    if(nrow(signals) == 1){
      ...
      model.null <- capture_warnings_and_output(lmer(as.numeric(gene_exp[gene, ]) ~ # gene expression
                                                       ...
                                                       geno.int[, as.character(pairs.int[i, 2])]*covariates[,1] + #  cell prop interaction term 
...
saveRDS(results, paste0(dir, "output_data/interactions_flanders_2/neutrophil_int_and_time_results_flanders_2.rds"))
